...
class net:
    def __init__(self, hidden=200, lr=0.00001):
        device = torch.device("cuda:0")
        self.n1 = torch.nn.LSTM(428032, hidden, 2).to(device)  # resnet50 428032 34 18 107008
        ...
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.opt, step_size=20, gamma=0.98)
    ...
    def __GetDataSet1(self, dirpath=None):
        ...
        device = torch.device("cuda:0")
        if dirpath is None:
            dirpath = os.path.dirname(__file__) + os.sep + 'sample_720'
        ex = self.ex
    ...
    def train(self, epochNum=1000, batchNum=8, finalLoss=1e-5):
        '''
        epochNum:训练的次数
    ...
    def eval(self, samplePath, savePath='\\final\\2-9.pth'):
        self.n1.eval()
        self.n2.eval()
        # 切换到评估模式
...
if __name__=='__main__':
    ...
    i=1
    for i in range(1,67):
        samplePath='2-9\{}'.format(i)
        pred,label = n.eval(samplePath=samplePath)
...
np.savetxt("label2-9.csv",labels)
np.savetxt("pred2-9.csv",pred)
