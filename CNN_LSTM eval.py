...
class net:
    def __init__(self, hidden=200, lr=0.000001):
        # lstm
        device = torch.device("cuda:0")
    ...
    def __GetDataSet1(self, dirpath=None):
        ...
        device = torch.device("cuda:0")
        if dirpath is None:
            dirpath = os.path.dirname(__file__) + os.sep + '15'
        ex = self.ex
    ...
    def train(self, epochNum=100, batchNum=2, finalLoss=1e-5):
        '''
        epochNum:训练的次数
...
# 计算 NSE
numerator = torch.sum((label - pred) ** 2)
denominator = torch.sum((label - label_mean) ** 2)
nse = 1 - numerator / denominator

print("NSE:", nse.item())  # 将结果转换为标量并打印
#np.savetxt("label4.csv",labels)
