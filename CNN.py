...
class net:
    def __init__(self,hidden=200,lr=0.000001):
        ...
        #一维卷
        self.n1 = torch.nn.Conv1d(450560, hidden, kernel_size=3, stride=1, padding=1).to(device)
        #全连接层
        self.n2 = torch.nn.Linear(59, 1).to(device)
        #再一个全连接 ps：凑形状用
        self.n3 = torch.nn.Linear(hidden,1).to(device)
    ...
    def forward(self, x):
        # 假设x的形状是[59, 2, 450560]
        x = x.permute(1, 2, 0)  # 现在x的形状是[2, 450560, 59]
        # 然后你可以继续你的计算
        return x
